...
const SearchEntriesCard: React.FC<SearchEntriesCardProps> = ({ entry, sx, isSelected = false, onDoubleClick, disableHoverEffect: _disableHoverEffect = false, hideTopBorderOnHover: _hideTopBorderOnHover = false, index: _index, onNavigateToTab, id_token = '', onRequestAccess}) => {
  ...
  const debouncedCheckAccess = useMemo(
    () => debounce((entryName: string, token: string, email: string) => {
      dispatch(checkEntryAccess({ entryName, id_token: token, userEmail: email }));
    }, 600),
    [dispatch]
  );
...
//   };
  ...
  useEffect(() => {
    ...
    const rawType = entry.entryType || '';
    setEntryType(
      rawType.includes('-')
        ? (rawType.split('-').pop() || '')  // 'bigquery-table' → 'table' (capitalizeFirstLetter applied in render)
        : (rawType && !rawType.includes('/') && !rawType.includes(':'))
    ...
    // Compute parent dataset from fullyQualifiedName (most reliable for BigQuery)
    // FQN format: bigquery:project.dataset.table
    const fqn: string = entry.fullyQualifiedName ?? '';
    const fqnMatch = fqn.match(/bigquery:[^.]+\.([^.]+)\./);
    if (fqnMatch) {
      setParentOrigin(fqnMatch[1]);
    } else {
      // Fallback: extract from resource path
      const resource: string = entry.entrySource?.resource ?? '';
      const dsMatch = resource.match(/datasets\/([^/]+)/);
      setParentOrigin(dsMatch ? dsMatch[1] : '');
    }
  }, [entry]);
        ...
        <Box
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: '1 1 auto', minWidth: 0, overflow: 'hidden' }}>
              ...
              {parentOrigin && (
                <Tooltip title={`Dataset: ${parentOrigin}`} arrow placement='top'>
                  <span style={{
                    color: mode === 'dark' ? '#dedfe0' : '#575757',
                    fontSize: "12px",
                    fontWeight: 400,
